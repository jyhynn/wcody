 ...
 */
@WebServlet("/wcody/notice_list.do")
public class NoticeListAction extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String pageMenu = Paging.getPaging("notice_list.do", nowPage, row_total, Common.QnA.BLOCKLIST, Common.QnA.BLOCKPAGE);
		//paging클래스가 페이징처리 알아서 해줌.
		
		request.setAttribute("pageMenu", pageMenu);
		request.getSession().removeAttribute("show");
		request.setAttribute("list", list);
		RequestDispatcher disp = request.getRequestDispatcher("w06_menu_notice.jsp");
